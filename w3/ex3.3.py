...
movies_df = pd.read_table('ex3.4/ml-1m/movies.dat', delimiter="::", header=None,
                    engine='python', names=['movie id', 'title', 'genre'])

users_df = pd.read_table('ex3.4//ml-1m/users.dat', delimiter="::", header=None,
                    engine='python', names=['user id', 'gender', 'age', 'occupation code', 'zip'])

ratings_df = pd.read_table('ex3.4//ml-1m/ratings.dat', delimiter="::", header=None,
...
# print(movie_data.sort('count', ascending=False).drop_duplicates('movie id').ix[:, ['title', 'count']].head())
...
# print(active_titles_f.sort('avg_rating', ascending=False).drop_duplicates('movie id').ix[:, ['title', 'avg_rating']].head(3))
# print(active_titles_m.sort('avg_rating', ascending=False).drop_duplicates('movie id').ix[:, ['title', 'avg_rating']].head(3))
...
print(active_titles.sort('rating_std', ascending=False).drop_duplicates('movie id').ix[:, ['title', 'rating_std']].head(5))